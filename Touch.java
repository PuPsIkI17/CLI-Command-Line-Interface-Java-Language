...
public class Touch implements Command {
	...
	public Touch(Position position, String parameter, PrintWriter out, PrintWriter err) {
	...
	public int execute() {
		...
		if (strend != -1) {
			...
			if (succes == 1) {
				err.println("touch: " + parameter + ": No such directory");
				position.set_nod(pos_nod_aux);
				return 1;
			}
		}

		Nod child = new File(child_name, position.get_nod(), out);
		int succes = position.get_nod().add(child);
		if (succes == 1) {
			Nod another_nod = position.get_nod().find_all(child_name);
			err.println("touch: cannot create file " + another_nod.get_absolute_path() + ": Node exists");
			position.set_nod(pos_nod_aux);
			return 2;
		}
	...
	public void search(Nod nod, String[] names, String child_name, int k, int lung, Position position) {
		...
		// daca s-a ajuns la pathul unde se face inserarea
		if (k == lung) {
			Nod aux_nod = position.get_nod();
			position.set_nod(nod);
			Instance instance = new Instance(out, err);
			Command command = instance.CreateCommand("touch", position, child_name);
			command.execute();
